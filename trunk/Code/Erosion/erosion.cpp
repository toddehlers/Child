  ...
  bedErode(0), sedTrans(0)
{
  ...
  bool VarKD;
  double steadykd;
  VarKD = infile.ReadBool( "OPTLITHO" ); // BJY 22012
  ...
  {
    steadykd = infile.ReadItem( kd, "KD" );  // Hillslope diffusivity coefficient
...
void tErosion::Diffuse( double rt, bool noDepoFlag, tMesh<class tLNode> *mp )
{
  tLNode * cn;
  tEdge * ce;
  double volout,  // Sediment volume output from a node (neg=input)
    ...
    kdd;  //Local diffusivity BJY 22012
...
//  if( kd==0 ) return;  BJY  commented out
  ...
  for( cn=nodIter.FirstP(); nodIter.IsActive(); cn=nodIter.NextP() )
      ...
      //  kdd is the spatially varaible kd value,  need to set to getKD.  All 'kdd' variables below in Diffuse where changed from 'kd' by BJY 22012
   // kdd=kd; //set kd value- to make spatially variable for each node later BJY 22012
...
//                std::cout << "KD is " << kdd << " \n";
      ...
      assert( kdd > 0.0 );
      delt = kEpsOver2 * ce->getLength()*ce->getLength() / kdd;
      if( delt < dtmax )
	{
	  dtmax = delt;
	  if(0) { //DEBUG
  ...
  do
  {
     ...
     for( ce=edgIter.FirstP(); edgIter.IsActive(); ce=edgIter.NextP() )
     {
        ...
        cn = static_cast<tLNode *>(ce->getOriginPtrNC());
        kdd = cn->getKD();
        volout = kdd*ce->CalcSlope()*ce->getVEdgLen()*dtmax;
