 ...
 **          (Jan 2000)
 **		-BJY made hillslope diffusivity a local property March, 2012
...
public:
  ...
  inline double getKD() const; //BJY 0312
  inline void setKD( double ); //BJY 0312
  // BJY!!!! currentlithoindex currentlithounit nextcontactdepth
  inline int getCurrentLithoIndex() const; //BJY 0312
  inline void setCurrentLithoIndex( int ); //BJY 0312
  inline int getCurrentLithoUnit() const; //BJY 0312
  inline void setCurrentLithoUnit( int ); //BJY 0312
  inline double getNextContactDepth() const; //BJY 0312
  inline void setNextContactDepth( double ); //BJY 0312

  
   // NOTE - For the get and set functions which involve arrays of size numlitholayers,
  // the arrays go from 0 to (numlitholayers-1) and must be indexed in this manner
  // BJY March 2012
  inline int getNumlithounits() const;
  inline void setNumlithounits( size_t ) const;
   
  inline void setLithoUnitList( unsigned int, int );
  inline int getLithoUnitList( unsigned int ) const;
//  inline tArray< int > const & getLithoUnitList( ) const;
  inline void setLithoContactDepths( unsigned int, double );
  inline double getLithoContactDepths( unsigned int ) const;
//  inline tArray< double > const & getLithoContactDepths( ) const;
...
protected:
  ...
  static size_t numlithounits;     //BJY variable for the number of lithology units for each node, so this will control the size of teh lithounit/lithocontactdepth arrays initialized below
  double qs;                        /* Sediment transport rate*/
  double kd;                          //BJY hillslope diffusivity
  int currentlithoindex;					// BJY lithology index (1: maximum number of layers beneath a given node point)
  int currentlithounit;                   // BJY current lithology unit value (integear value corresponds to lookup table read in tLithology)
  double nextcontactdepth;                // elevation of the next contact location.  When compared to   
  tArray< double > qsm;             /* multi size; transport rate of each size fraction*/
  tArray< int > lithounit;                      //BJY integer reference to lithology parameters (kd, kf, etc)
  tArray< double > lithocontactdepth;				//BJY depth of lithology contacts 
...
// BJY 0312  
inline double tLNode::getKD() const
{
	return kd;
}
// BJY 0312
inline void tLNode::setKD( double newkd)
{
	kd = newkd;
}
// BJY lithology index (1: maximum number of layers beneath a given node point)

inline int tLNode::getCurrentLithoIndex() const
{
	return currentlithoindex;
}
// BJY 0312 currentlithoindex currentlithounit nextcontactdepth
inline void tLNode::setCurrentLithoIndex( int newindex)
{
	currentlithoindex = newindex;
}
inline int tLNode::getCurrentLithoUnit() const
{
	return currentlithounit;
}
// BJY 0312 
inline void tLNode::setCurrentLithoUnit( int newunit)
{
	currentlithounit = newunit;
}

inline double tLNode::getNextContactDepth() const
{
	return nextcontactdepth;
}
// BJY 0312
inline void tLNode::setNextContactDepth( double newdepth)
{
	nextcontactdepth = newdepth;
}
//end BJY!!!

inline void tLNode::setLithoUnitList( unsigned int i, int val )
{
	//std::cout << "setLithoUnitsList = " << i << val  << std::endl;
//  if(unlikely(i>=numlithounits))
//    ReportFatalError( "Trying to index lithology layers that don't exist ");
  lithounit[i] = val;
  
}

inline void tLNode::setLithoContactDepths( unsigned int i, double val )
{
//  if(unlikely(i>=numlithounits))
//    ReportFatalError( "Trying to index contact depths that don't exist ");
  lithocontactdepth[i] = val;
  
}

inline int tLNode::getLithoUnitList( unsigned int i ) const
{
...
inline double tLNode::getLithoContactDepths( unsigned int i ) const
{
 
  return lithocontactdepth[i];
...
// added by BJY March 2012 to define the number of lithology units, similar to defining number of grain sizes numlithounits
inline int tLNode::getNumlithounits() const
{
  return numlithounits;
}

inline void tLNode::setNumlithounits( size_t val ) const
{
  numlithounits = val;
}
