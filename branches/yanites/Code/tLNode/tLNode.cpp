 ...
 **       source of the problem noted by nmg below. (gt, 2/2000)ƒ
 **
\**************************************************************************/
...
dzdt(0.), drdt(0.), tau(0.), taucb(0.), taucr(0.), qs(0.), kd(0.),
currentlithoindex(0), currentlithounit(0), nextcontactdepth(0),
qsm(),
lithounit(),
lithocontactdepth(),
qsin(0.),
...
public1(-1)
{
  ...
  qsm.setSize( numg );
  qsinm.setSize( numg );
     int numlithounits;  // BJY!!!!!!!
      numlithounits = infile.ReadItem( numlithounits, "MAXLITHOUNITS" );  // BJY!!!!!!!
//  int nlitho;
//    std::cout << "SETSIZETEST = " << numlithounits << std::endl;
    lithounit.setSize(numlithounits); // BJY!!!!!!!
    lithocontactdepth.setSize(numlithounits);  // BJY!!!!!!!

  
  // BJY March 2012  set size of lilthounits and contact depth node properties
//   int lithlayn;
//   lithlayn = infile.ReadItem( lithlayn, "MAXLITHOUNITS" );
//   numlithounits = lithlayn;
//     lithounit.setSize(numlithounits);
//	lithocontactdepth.setSize(numlithounits);
  ...
  if(!optReadLayer){
	...
	bool VarKB;
	VarKB = infile.ReadBool( "OPTLITHO" ); // BJY 22012
	if(VarKB)
	{
	...
	// THINK ABOUT THIS BRIAN, MIGHT WANT TO SET KR IN LITHOLOGY AS WELL, BUT FOR NOW LITHOLOGY WILL BE ALL BEDROCK
	if(VarKB)
	{
...
tLNode::tLNode( const tLNode &orig )                               //tLNode
  : tNode( orig ), vegCover( orig.vegCover ), rock( orig.rock ),
    ...
    dzdt(orig.dzdt), drdt(orig.drdt), tau(orig.tau), taucb(orig.taucb), taucr(orig.taucr), qs(orig.qs), kd(orig.kd),
    currentlithoindex(orig.currentlithoindex), currentlithounit(orig.currentlithounit), nextcontactdepth(orig.nextcontactdepth),
    qsm(orig.qsm),
    lithounit(orig.lithounit),
	lithocontactdepth(orig.lithocontactdepth),
    qsin(orig.qsin),
    qsinm(orig.qsinm ),
    qsdin(orig.qsdin),
    qsdinm(orig.qsdinm ),
  ...
  if( &right != this )
    {
	  ...
	  taucr = right.taucr;
	  kd = right.kd;
	  currentlithoindex = right.currentlithoindex;
	  currentlithounit = right.currentlithounit;
	  nextcontactdepth = right.nextcontactdepth;
      uplift = right.uplift;
      qsm = right.qsm;
      lithounit = right.lithounit;
	  lithocontactdepth = right.lithocontactdepth;
      qsinm = right.qsinm;
      qsdinm = right.qsdinm;
...
void tLNode::TellAll() const
{
  ...
  if( getEdg() ) {
	...
	for( size_t i=0; i<numg; i++)
      ...
      std::cout << " kdiffuse: " << getKD() << std::endl;
