...
Initialize( int argc, char **argv )
{
   ...
   **    Get options for erosion type, meandering, etc.
   ...
   optVarLithology = inputFile.ReadBool( "OPTLITHO" ); // BJY 22012
   ...
   if( optLoessDep )
       loess = new tEolian( inputFile );
        ...
        // If applicable, create lithology object  BJY 22012
   if( optVarLithology )
       lithology = new tLithology( mesh, inputFile );
...
RunOneStorm()
{
	...
	**********************************************************************/
	...
	if( optStratGrid )
 	...
 	if( optVarLithology )
		lithology->UpdateLithology( mesh, storm->getStormDuration() + storm->interstormDur());
	...
	if( optNonlinearDiffusion )
		...
		erosion->Diffuse( storm->getStormDuration() + storm->interstormDur(),
						  optDiffuseDepo, mesh );  //edited by BJY for variable KD
